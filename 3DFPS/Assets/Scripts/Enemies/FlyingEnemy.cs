...
public class FlyingEnemy : Enemy
{
    ...
    [Tooltip("The speed of the rotation once the enemy is engaged")]
    [Range(0f, 5f)]
            ...
            else
            {
                switch (stopBehavior)
                {
                    ...
                    case BehaviorAtStopDistance.CircleClockwise:
                        return transform.position + Vector3.Cross((target.position - transform.position), transform.up).normalized * rotationSpeed * Time.deltaTime;
